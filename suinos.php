...
if (!isset($_SESSION['usuario_id'])) {
    die("Você precisa estar logado para cadastrar um Suino.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($nome) || empty($alimentacao) || empty($idade)) {
        ...
        $sql = "INSERT INTO suinos (nome, alimentacao, idade, producao, user_id) 
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM suinos WHERE id = $animal_id AND user_id = {$_SESSION['usuario_id']}";

    if ($conn->query($sql) === TRUE) {
        $erro = "Suino excluído com sucesso!";
    } else {
...
$sql = "SELECT * FROM suinos WHERE user_id = {$_SESSION['usuario_id']}";
$result = $conn->query($sql);

$suinoCadastrados = [];
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $suinoCadastrados[] = $row;
    }
}
?>
...
<head>
  ...
  <title>Cadastro de Suinos</title>
  <style>
  ...
  <div class="container">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        <label for="nome">Nome do Suino</label>
    ...
    <div class="list-container">
      <?php foreach ($suinoCadastrados as $suinos): ?>
        <div class="list-item">
          <strong>Nome:</strong> <?php echo $suinos['nome']; ?><br>
          <strong>Alimentação:</strong> <?php echo $suinos['alimentacao']; ?><br>
          <strong>Idade:</strong> <?php echo $suinos['idade']; ?><br>
          <strong>Produção:</strong> <?php echo $suinos['producao']; ?><br>
          <a href="?delete=<?php echo $suinos['id']; ?>" class="delete-button" onclick="return confirm('Tem certeza que deseja excluir este suino?');">Excluir</a>
          </div>
      <?php endforeach; ?>
    </div>
  </div>
