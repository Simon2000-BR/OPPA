...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add' && isset($_POST['nome'], $_POST['alimentacao'], $_POST['idade'])) {
        ...
        if (empty($nome) || empty($alimentacao) || empty($idade)) {
            ...
            $sql = "INSERT INTO suinos (nome, alimentacao, idade, producao) VALUES ('$nome', '$alimentacao', $idade, '$producao')";

            if ($conn->query($sql) === TRUE) {
                $erro = "Novo suíno cadastrado com sucesso!";
            } else {
...
// Processa a exclusão de um suíno
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $delete_sql = "DELETE FROM suinos WHERE id = $id";
    if ($conn->query($delete_sql) === TRUE) {
        header("Location: suinos.php");
        exit;
    } else {
        $erro = "Erro ao excluir suíno: " . $conn->error;
    }
}

// Recupera os suínos cadastrados para exibição
$result = $conn->query("SELECT * FROM suinos");
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $suinosCadastrados[] = $row;
    }
}
...
<head>
  ...
  <title>Cadastro de Suínos</title>
  <style>
  ...
  <div class="container">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        <label for="nome">Nome do Suíno</label>
        <input type="text" id="nome" name="nome" placeholder="Digite o nome do suíno" required>
      </div>
      <div class="form-group">
      ...
      <button type="submit">Adicionar Suíno</button>
    </form>

    <!-- Exibe os suínos cadastrados -->
    <div class="list-container">
      <?php if (count($suinosCadastrados) > 0): ?>
        <?php foreach ($suinosCadastrados as $suino): ?>
          <div class="list-item">
            <strong>Nome:</strong> <?php echo $suino['nome']; ?><br>
            <strong>Alimentação:</strong> <?php echo $suino['alimentacao']; ?><br>
            <strong>Idade:</strong> <?php echo $suino['idade']; ?><br>
            <strong>Produção:</strong> <?php echo $suino['producao']; ?>
            <a href="?delete=<?php echo $suino['id']; ?>" class="delete-button" onclick="return confirm('Tem certeza que deseja excluir?')">Excluir</a>
          </div>
        <?php endforeach; ?>
      <?php else: ?>
        <p>Nenhum suíno cadastrado.</p>
      <?php endif; ?>
    </div>
  </div>
</body>
