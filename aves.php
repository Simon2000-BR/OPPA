...
if (!isset($_SESSION['usuario_id'])) {
    die("Você precisa estar logado para cadastrar uma ave.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($nome) || empty($alimentacao) || empty($idade)) {
        ...
        $sql = "INSERT INTO aves (nome, alimentacao, idade, producao, user_id) 
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM aves WHERE id = $animal_id AND user_id = {$_SESSION['usuario_id']}";

    if ($conn->query($sql) === TRUE) {
        $erro = "Ave excluído com sucesso!";
    } else {
...
$sql = "SELECT * FROM aves WHERE user_id = {$_SESSION['usuario_id']}";
...
<body>
  ...
  <div class="container">
    ...
    <div class="list-container">
      <?php foreach ($avesCadastrados as $aves): ?>
        <div class="list-item">
          <strong>Nome:</strong> <?php echo $aves['nome']; ?><br>
          <strong>Alimentação:</strong> <?php echo $aves['alimentacao']; ?><br>
          <strong>Idade:</strong> <?php echo $aves['idade']; ?><br>
          <strong>Produção:</strong> <?php echo $aves['producao']; ?><br>
          <a href="?delete=<?php echo $aves['id']; ?>" class="delete-button" onclick="return confirm('Tem certeza que deseja excluir está ave?');">Excluir</a>
          </div>
      <?php endforeach; ?>
    </div>
  </div>
