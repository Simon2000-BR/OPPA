...
if (!isset($_SESSION['usuario_id'])) {
    die("Você precisa estar logado para cadastrar um bovino.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($nome) || empty($alimentacao) || empty($idade)) {
        ...
        $sql = "INSERT INTO bovinos (nome, alimentacao, idade, producao, user_id) 
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM bovinos WHERE id = $animal_id AND user_id = {$_SESSION['usuario_id']}";

    if ($conn->query($sql) === TRUE) {
        $erro = "Bovino excluído com sucesso!";
    } else {
...
$sql = "SELECT * FROM bovinos WHERE user_id = {$_SESSION['usuario_id']}";
  ...
  <div class="container">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        <label for="nome">Nome da Bovinos</label>
    ...
    <div class="list-container">
      <?php foreach ($bovinosCadastrados as $bovinos): ?>
        <div class="list-item">
          <strong>Nome:</strong> <?php echo $bovinos['nome']; ?><br>
          <strong>Alimentação:</strong> <?php $bovinos['alimentacao']; ?><br>
          <strong>Idade:</strong> <?php echo $bovinos['idade']; ?><br>
          <strong>Produção:</strong> <?php echo $bovinos['producao']; ?><br>
          <a href="?delete=<?php echo $bovinos['id']; ?>" class="delete-button" onclick="return confirm('Tem certeza que deseja excluir este bovino?');">Excluir</a>
          </div>
      <?php endforeach; ?>
    </div>
  </div>
