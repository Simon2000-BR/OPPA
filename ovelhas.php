...
if (!isset($_SESSION['usuario_id'])) {
    die("Você precisa estar logado para cadastrar uma ovelha.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($nome) || empty($alimentacao) || empty($idade)) {
        ...
        $sql = "INSERT INTO ovelhas (nome, alimentacao, idade, producao, user_id) 
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM ovelhas WHERE id = $animal_id AND user_id = {$_SESSION['usuario_id']}";

    if ($conn->query($sql) === TRUE) {
        $erro = "Ovelha excluído com sucesso!";
    } else {
...
$sql = "SELECT * FROM ovelhas WHERE user_id = {$_SESSION['usuario_id']}";
$result = $conn->query($sql);

$ovelhasCadastrados = [];
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $ovelhasCadastrados[] = $row;
    }
}
?>
...
<body>
  ...
  <div class="container">
    ...
    <div class="list-container">
      <?php foreach ($ovelhasCadastrados as $ovelhas): ?>
        <div class="list-item">
          <strong>Nome:</strong> <?php echo $ovelhas['nome']; ?><br>
          <strong>Alimentação:</strong> <?php echo $ovelhas['alimentacao']; ?><br>
          <strong>Idade:</strong> <?php echo $ovelhas['idade']; ?><br>
          <strong>Produção:</strong> <?php echo $ovelhas['producao']; ?><br>
          <a href="?delete=<?php echo $ovelhas['id']; ?>" class="delete-button" onclick="return confirm('Tem certeza que deseja excluir esta ovelha?');">Excluir</a>
          </div>
      <?php endforeach; ?>
    </div>
  </div>
